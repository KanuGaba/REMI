...
class WakeWordDetector(Thread):
    ...
    def run(self):
        ...
        for x in self._keyword_file_paths:
            keyword_names.append(os.path.basename(x).replace('.ppn', '').replace('_compressed', '').split(' '))
        ...
        try:
            ...
            while True:
                ...
                if num_keywords == 1 and result:
                    ...
                    if (self._detection_callback != None):
...
def startWakeWordDetection(detection_callback, window):
    ...
    if args.show_audio_devices_info:
        ...
        if args.keyword_file_paths is None:
            ...
            if currOS == "Darwin":
                keyword_file_paths = 'wake_word_detector/resources/keyword_files/mac/hey_remi_mac.ppn'
            elif currOS == "Linux":
                keyword_file_paths = 'wake_word_detector/resources/keyword_files/linux/hey_remi_linux.ppn'
            else:   # Default to windows.
                keyword_file_paths = 'wake_word_detector/resources/keyword_files/windows/hey_remi_windows.ppn'
